@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-center mt-4">
        <div class="inline-flex items-center gap-3">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center justify-center w-10 h-10 text-cyan-700 bg-white border border-gray-100 rounded-md shadow-sm hover:bg-cyan-50" aria-label="Previous">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </a>
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            @else
                                <li>
                                    <a href="{{ $url }}" class="px-3 py-2 min-w-[40px] text-sm text-center text-gray-700 bg-white border border-gray-100 rounded-md hover:bg-cyan-50">{{ $page }}</a>
                                </li>
                            @endif
                        @endforeach
                    @endif
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center justify-center w-10 h-10 text-cyan-700 bg-white border border-gray-100 rounded-md shadow-sm hover:bg-cyan-50" aria-label="Next">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
