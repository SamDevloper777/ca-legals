        ...
        <div class="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div>
                ...
                <div class="mt-6 flex flex-col sm:flex-row gap-3">
                    <a href="#contact-form" class="inline-block bg-cyan-700 text-white px-6 py-3 rounded-lg font-medium shadow hover:bg-cyan-700 transition w-full sm:w-auto text-center">Request Consultation</a>
            ...
            <form id="contact-form" action="/contact" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="name" value="{{ old('name') }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-200" placeholder="Your name">
                        @error('name') <div class="text-red-600 text-sm mt-1">{{ $message }}</div> @enderror
                    </div>
                    <div>
                        <label class="block text-sm text-gray-700 mb-1">Email</label>
                        <input name="email" value="{{ old('email') }}" type="email" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-200" placeholder="you@example.com">
                ...
                <div>
                    ...
                    <input name="subject" value="{{ old('subject') }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-200" placeholder="Brief subject">
                </div>

                <div>
                    <label class="block text-sm text-gray-700 mb-1">Message</label>
                    <textarea name="message" rows="6" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-200" placeholder="How can we help?">{{ old('message') }}</textarea>
                    @error('message') <div class="text-red-600 text-sm mt-1">{{ $message }}</div> @enderror
                </div>

                <div class="flex items-center gap-4">
                    <button type="submit" class="bg-cyan-700 text-white px-6 py-3 rounded-lg font-medium hover:bg-cyan-700 transition w-full sm:w-auto">Send Message</button>
                </div>
            </form>
        </div>
